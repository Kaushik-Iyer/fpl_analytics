# app/services/gameweek_stats_importer.rb
class GameweekStatsImporter
    def self.import_for_player(player_id)
      ...
      history_data.each do |stat|
        GameweekStat.find_or_create_by!(
          player_id: player_id,
    ...
    def self.import_all_players
      Player.find_each.with_index do |player, index|
        import_for_player(player.id)
        sleep(0.5) # Rate limiting
        Rails.logger.info("Imported stats for player #{player.id} (#{index + 1}/#{Player.count})")
      rescue ImportError => e
        Rails.logger.error("Skipping player #{player.id}: #{e.message}")
        next
      end
    end
